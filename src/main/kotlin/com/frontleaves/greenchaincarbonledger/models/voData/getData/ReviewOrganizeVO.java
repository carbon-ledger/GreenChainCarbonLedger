...
public class ReviewOrganizeVO {
    ...
    @Pattern(regexp = "^data:image/(png|jpg|jpeg|gif);base64,[A-Za-z0-9+/=]+$", message = "营业执照必须是 base64 图像数据")
    ...
    @Pattern(regexp = "^data:image/(png|jpg|jpeg|gif);base64,[A-Za-z0-9+/=]+$", message = "法人身份证正面照必须是 base64 图像数据")
    public String legalIdCardFront;
    @Pattern(regexp = "^data:image/(png|jpg|jpeg|gif);base64,[A-Za-z0-9+/=]+$", message = "法人身份证反面照必须是 base64 图像数据")
