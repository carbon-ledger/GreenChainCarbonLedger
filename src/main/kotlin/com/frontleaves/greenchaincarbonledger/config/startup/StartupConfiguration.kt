...
open class StartupConfiguration(
    ...
    open fun sqlDataPreparation(): CommandLineRunner {
        return CommandLineRunner {
            ...
            val prepareData = PrepareData(jdbcTemplate)
            // 检查角色数据库是否完整
            val getRoleList = jdbcTemplate.query("SELECT * FROM fy_role") { rs, _ ->
            ...
            // 检查权限数据库是否完整
            val getPermissionList = jdbcTemplate.query("SELECT * FROM fy_permission") { rs, _ ->
